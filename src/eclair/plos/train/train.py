...
from eclair.plos.model.model import Binary_model, MinkUNet14C
from eclair.plos.model.utils import collate_custom_test, seed_everything


def make_model(conf) -> MinkUNet14C:  # MinkUnet34a pour basemodel
    """
...
def make_data(conf) -> PyGDataLoader:
    ...
    """
    label_file = "/home/mtardif/src/MinkowskiEngine/data/labels_v13_horssol_filetred_.json"
    # label_file = "/mnt/d/pointcloud_data/iantsa/reanno_16c/gt/labels_iantsa16c.json"
    with open(label_file, encoding="utf-8") as f:
        all_tiles: list[dict] = json.load(f)

    train_tiles = [
        "/home/mtardif/src/MinkowskiEngine/data/PLO" + "/" + x["tile_name"]  # Mettre à jour si basemodel
        for x in all_tiles
        if x["split"] == "train"
    ]
...
def train_model(model, device, conf, train_data, val_data) -> None:
    ...
    torch.save(model.state_dict(), f"model_weights/{conf.weights_name}_finalweights.pth")
...
def main() -> None:
    ...
    base_conf = OmegaConf.load("plo.yaml")  # first_model.yaml si basemodel
