...
class NormalizeCoordinates(BaseTransform):
    ...
    - Filtrant les points hors de la plage Z en utilisant un quantile pour le `minz`.
    - Ajoutant un point artificiel (x_max, y_max, z_max) avec RGB et classification.
    """

    def __init__(self, max_range_xy=40.0, max_range_z=40.0, z_quantile=0.5):
        super().__init__()
    ...
    def __call__(self, data):
        ...
        # 1️⃣ Recherche du `minz` filtré avec un quantile
        z = coords[:, 2]
        ...
        fake_point = (min_xyz_clean + torch.tensor(
            [self.max_range_xy, self.max_range_xy, self.max_range_z], device=coords.device
        )).unsqueeze(0)
...
class NormalizeCoordinates_predict(BaseTransform):
    ...
    def __call__(self, data):
        ...
        try:
            # 1️⃣ Calcul du min propre (ignore les z_quantile% plus bas)
            z = coords[:, 2]
            ...
            if "quantile() input tensor is too large" in str(e):
                print("Erreur: Tenseur trop grand pour quantile(). Utilisation du min global de Z.")
                min_xyz_clean = coords.min(dim=0).values  # Utilise le min global
                data.index_quantile_excluded = torch.tensor([], dtype=torch.long,
                                                            device=coords.device)  # Ensemble vide sous forme de tensor
            else:
                # Relancer l'exception si ce n'est pas l'erreur attendue
                raise

        # 2️⃣ Définition de la bbox 40x40x40 à partir du min filtré
        max_xyz = min_xyz_clean + torch.tensor(
            [self.max_range_xy, self.max_range_xy, self.max_range_z], device=coords.device
        )
        mask_bbox = torch.all((coords >= min_xyz_clean) & (coords <= max_xyz), dim=1)

        # 3️⃣ Sélection des points dans la bbox
        coords_kept = coords[mask_bbox]
        index_kept = index_original[mask_bbox]

        # 4️⃣ Ajout du point fictif
        fake_point = (min_xyz_clean + torch.tensor(
            [self.max_range_xy, self.max_range_xy, self.max_range_z], device=coords.device
        )).unsqueeze(0)
        ...
        # 6️⃣ Indices pour reconstruction
        ...
        # 7️⃣ Remplacement des coordonnées dans `data`
