...
# class HDF5SuperVoxelDataset(Dataset):
#     """
#     Dataset pour lire un fichier HDF5 prétraité contenant les supervoxels.
#     Chaque groupe dans le HDF5 représente un nuage de points.
#     """
#
#     def __init__(self, hdf5_file: str, cloud_name: str = None):
#         """
#         Initialise le dataset.
#
#         :param hdf5_file: Chemin du fichier HDF5 contenant les supervoxels.
#         :param cloud_name: Nom du nuage à charger (correspondant à un groupe dans le fichier HDF5).
#                            Si None, charge tous les nuages (utilisation entraînement).
#         """
#         self.hdf5_file = hdf5_file
#         self.h5f = h5py.File(hdf5_file, 'r')
#
#         if cloud_name:
#             if cloud_name not in self.h5f:
#                 raise KeyError(f"Le nuage {cloud_name} n'existe pas dans {hdf5_file}")
#             self.cloud_keys = [cloud_name]  # Charge uniquement ce nuage
#         else:
#             self.cloud_keys = list(self.h5f.keys())  # Charge tous les nuages
#
#     def __len__(self):
#         return len(self.cloud_keys)
#
#     def __getitem__(self, idx):
#         cloud_key = self.cloud_keys[idx]
#         group = self.h5f[cloud_key]
#         raw_points_list = []
#         indices_list = []
#
#         # Pour chaque supervoxel (chaque sous-groupe)
#         for sv_key in group.keys():
#             sv_group = group[sv_key]
#             # Utilisation de [()] pour charger l'intégralité du dataset
#             data = sv_group["data"][()]       # Tableau de forme (n_points, 7)
#             idx_arr = sv_group["indices"][()]   # Tableau des indices d'origine des points
#
#             raw_points_list.append(data)
#             indices_list.append(idx_arr)
#
#         if len(raw_points_list) > 0:
#             all_points = np.concatenate(raw_points_list, axis=0)
#             all_indices = np.concatenate(indices_list, axis=0)
#         else:
#             all_points = np.empty((0, 7))
#             all_indices = np.empty((0,), dtype=int)
#
#         # Extraction des champs
#         pos = torch.tensor(all_points[:, :3], dtype=torch.float32)         # Coordonnées normalisées
#         x = torch.tensor(all_points[:, 3:6], dtype=torch.float32)            # Features (RGB normalisés)
#         classification = torch.tensor(all_points[:, 6], dtype=torch.long)    # Classe
#         indices_tensor = torch.tensor(all_indices, dtype=torch.long)         # Indices d'origine
#
#         return Data(pos=pos, x=x, classification=classification, indices=indices_tensor)
#
#     import os
#     import random
#     from typing import Sequence, Optional, List, Union
...
#             # Convert targets to one-hot encoding if not already one-hot
#             if targets.ndim == inputs.ndim - 1:
#                 targets = nn.functional.one_hot(targets, num_classes=inputs.size(1)).permute(0, -1,
#                                                                                              *range(1, targets.ndim))
#
#             # Flatten inputs and targets across all dimensions except class dimension
#             inputs = inputs.view(inputs.size(0), inputs.size(1), -1)  # [B, C, N]
#             targets = targets.view(targets.size(0), targets.size(1), -1)  # [B, C, N]
...
#         for sv_key in group.keys():
#             sv_group = group[sv_key]
#             # On suppose que chaque supervoxel est stocké dans un sous-groupe avec deux datasets : "data" et "indices"
#             data = np.array(sv_group["data"])       # Tableau de forme (n_points, 7)
#             idx_arr = np.array(sv_group["indices"])   # Tableau des indices d'origine des points
#
#             raw_points_list.append(data)
#             indices_list.append(idx_arr)
#
