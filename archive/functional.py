...
#     """
#     Normalizes the coordinates of the point cloud data.
#
...
#
#
#         centroid: torch.Tensor = coordinates.mean(0)
#         coordinates -= centroid
#         furthest_distance: torch.Tensor = torch.max(
#             torch.sqrt(torch.sum(torch.abs(coordinates) ** 2, axis=-1))
#         )
#         coordinates /= furthest_distance
...
class NormalizeCoordinates(BaseTransform):
    """
    Normalise les coordonnées XYZ :
    - Centre horizontalement autour du barycentre (X,Y).
    - Soustrait explicitement l’altitude minimale du sol (axe Z).
    - Normalise la hauteur Z par une constante (ex : 50m).
    """

    def __init__(self, vertical_scale=40.0, horizontal_scale=None):
        super().__init__()
        self.vertical_scale = vertical_scale
        self.horizontal_scale = horizontal_scale  # optionnel

    def __call__(self, data: Data) -> Data:
        coords = data.xyz.clone()

        # 1. Centrage horizontal (X,Y)
        coords[:, :2] -= coords[:, :2].mean(dim=0)

        # 2. Échelle horizontale (si précisée)
        if self.horizontal_scale is None:
            horizontal_scale = coords[:, :2].abs().max()
        else:
            horizontal_scale = self.horizontal_scale

        if horizontal_scale > 0:
            coords[:, :2] /= horizontal_scale

        # 3. Soustraire explicitement altitude sol (Z)
        coords[:, 2] -= coords[:, 2].min()

        # 4. Normaliser Z par constante (vertical_scale)
        coords[:, 2] /= self.vertical_scale

        data.pos = coords.float()
        del data.xyz
        return data
...
#     """
#
#     def __init__(self, max_range_xy=40.0, max_range_z=20.0):
#         super().__init__()
...
#         # coords = data.xyz
#
#         # 1️⃣ Trouver les bornes (x_min, y_min, z_min)
#         shift = data.xyz.new_tensor([data.xyz[:, 0].min(), data.xyz[:, 1].min(), data.xyz[:, 2].min()])
#         data.xyz.sub_(shift)  # op in-place
#
#         # 2️⃣ Filtrer les points en dehors de la plage Z
#         mask = data.xyz[:, 2] <= self.max_range_z
#         data.xyz = data.xyz[mask]
#
#         # 3️⃣ Ajouter un point artificiel garantissant la même extension
#         new_point = torch.tensor([[self.max_range_xy, self.max_range_xy, self.max_range_z]], dtype=data.xyz.dtype, device=data.xyz.device)
...
# class NormalizeCoordinates(BaseTransform):
#     def __init__(self, max_range_xy=40.0, max_range_z=30.0):
#         super().__init__()
#         self.max_range_xy = max_range_xy
#         self.max_range_z = max_range_z
#
#     def __call__(self, data):
#         # Trouver les minima en une seule passe
#         min_vals = torch.amin(data.xyz, dim=0)
#
#         # Décalage in-place
#         data.xyz.sub_(min_vals)
#
#         # Filtrer en Z en une seule passe optimisée
#         mask = data.xyz[:, 2] <= self.max_range_z
#         data.xyz = torch.masked_select(data.xyz, mask.unsqueeze(1)).reshape(-1, 3)
#
#         # Pré-allouer un buffer et ajouter un point artificiel
#         new_xyz = torch.empty((data.xyz.shape[0] + 1, 3), dtype=data.xyz.dtype, device=data.xyz.device)
#         new_xyz[:-1] = data.xyz
#         new_xyz[-1] = data.xyz.new_tensor([self.max_range_xy, self.max_range_xy, self.max_range_z])
#         data.xyz = new_xyz
#
#         # Mise à jour des autres attributs avec le même masquage
#         if hasattr(data, "rgb"):
#             data.rgb = data.rgb[mask]
#             new_rgb = data.rgb.new_zeros((1, 3))
#             data.rgb = torch.cat([data.rgb, new_rgb], dim=0)
#
#         if hasattr(data, "classification"):
#             data.classification = data.classification[mask]
#             new_classification = data.classification.new_zeros((1,))
#             data.classification = torch.cat([data.classification, new_classification], dim=0)
#
#         # Passer en float32 au lieu de double
#         data.pos = data.xyz.double()
#
#         # Supprimer `xyz` pour libérer de la mémoire si inutile
#         del data.xyz
#
#         return data
...
class NormalizeCoordinates_predict(BaseTransform):
    ...
    def __call__(self, data):
        ...
        # 2️⃣ Trouver les bornes (x_min, y_min, z_min)
        x_min, y_min, z_min = coords[:, 0].min(), coords[:, 1].min(), coords[:, 2].min()

        coords -= torch.tensor([x_min, y_min, z_min],
                               dtype=coords.dtype,
                               device=coords.device)
...
#         data.pos = coordinates.clone().to(torch.float64)
#         if isinstance(data.xyz, np.ndarray):
#             data.xyz = torch.tensor(data.xyz, dtype=torch.float64)
#         data.xyz = data.xyz.clone().to(torch.float64)  # Juste conversion, ne change rien normalement
#         return data
