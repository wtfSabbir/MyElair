...
def las2pyg(las: laspy.LasData, path: Path, config: DictConfig | ListConfig) -> Data:
    """
    ...
    return Data(
        ...
        if "intensity" in config['feature_names']
        else None,
        ...
        rgb=(
            ...
            if "colors" in config['feature_names']
            else None
        ),
        filename=path,
    )
...
class TrainDataset(torch.utils.data.Dataset):
    def __init__(self, config, fnames: Sequence[Path]):
        ...
        self.transforms = compose_transforms_from_list(config.train_transforms, global_config=config)
        self.config = config
    ...
    def __getitem__(self, index: int) -> Data:
        ...
        pyg_data: Data = las2pyg(las, str(tile_path), self.config)
        if self.transforms:
...
class ValDataset(torch.utils.data.Dataset):
    def __init__(self, config, fnames: Sequence[Path]):
        ...
        """
        self.fnames: Sequence[Path] = fnames
        self.transforms = compose_transforms_from_list(config.val_transforms, global_config=config)
        self.config = config
...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, config, fnames: Sequence[Path]):
        ...
        """
        self.fnames: Sequence[Path] = fnames
        self.transforms = compose_transforms_from_list(config.test_transforms, global_config=config)
        self.config = config
...
class FocalTverskyLoss(nn.Module):
    def __init__(self, alpha: float = 0.5, beta: float = 0.5, gamma: float = 1.0, smooth: int = 1):
        """
...
class FocalTverskyLoss_multiclass(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor):
        ...
        Tversky = (tp + self.smooth) / (tp + alpha * fp + beta * fn + self.smooth)  # Shape: [B, C]
