...
def compose_transforms_from_list(input_transforms: list[dict], global_config: dict = None) -> Compose:
    ...
    for transform in input_transforms:
        if transform["name"] in available_transforms:
            # Récupérer les params passés dans le YAML sous train_transforms
            params = transform.get("params", {})

            # Injection des params globaux pour NormalizeCoordinates
            if (
                transform["name"] == "NormalizeCoordinates" or transform["name"] == "NormalizeCoordinates_predict"
            ) and global_config is not None:
                norm_params = global_config.get("cloud_size", {})
                # Les params dans la liste train_transforms ont priorité sur la config globale
                params = {**norm_params, **params}

            transform_fn = available_transforms[transform["name"]](**params)
...
class Noise(BaseTransform):
    ...
    def __call__(self, data: Data) -> Data:
        if "intensity" in self.feature_names:
            random_noise_int: npt.NDArray[np.float32] = np.random.uniform(-0.05, 0.05, np.shape(data.intensity))
            new_intensity: npt.NDArray[np.float32] = data.intensity + data.intensity * random_noise_int
            data.intensity = new_intensity

        if "colors" in self.feature_names:
            random_noise_rgb: npt.NDArray[np.float32] = np.random.uniform(-0.05, 0.05, np.shape(data.rgb))
...
class NormalizeCoordinates(BaseTransform):
    ...
    def _compute_min_xyz_clean(self, coords: torch.Tensor) -> torch.Tensor:
        """
        Calcule le minimum XYZ filtré en ignorant les points en dessous d'un quantile sur Z.
        ...
        try:
            ...
            if "quantile() input tensor is too large" in str(e):
                print(f"Warning: quantile() failed due to large tensor size. Using coords.min() instead.")
    ...
    def __call__(self, data: Data) -> Data:
        ...
        if hasattr(data, "intensity"):
            int = data.intensity[mask_bbox].clone()
            int = torch.cat([int, torch.tensor([0], dtype=int.dtype, device=int.device)], dim=0)
            data.intensity = int
